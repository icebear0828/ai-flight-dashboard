...
本文档面向当前项目根目录公开入口。`docs/` 下的文档可能包含历史设计、路线图和内部说明，使用方式以本文档和 `README.md` 为准。
...
config.json                            # 应用配置
~/.ai-flight-dashboard/custom_pricing.json  # 自定义模型价格
```
...
3. 加载 `~/.ai-flight-dashboard/custom_pricing.json` 覆盖或补充模型价格。
