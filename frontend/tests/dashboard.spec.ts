...
test('settings device management can create edit clear and soft-delete devices', async ({ page }) => {
  ...
  await page.route('**/api/stats?*', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ periods, sources: [], devices: [], projects: [], is_paused: false }),
    });
  });
  await page.route('**/api/lan/scan', async (route) => {
  ...
  await page.route('**/api/config', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ auto_start: false, extra_watch_dirs: [], enable_lan: true }),
    });
  });
  await page.route('**/api/devices**', async (route) => {
    if (route.request().method() === 'DELETE') {
      ...
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ device_id: softDeletedDevice, superseded_count: 1 }),
      });
      return;
    }
    await route.fulfill({
